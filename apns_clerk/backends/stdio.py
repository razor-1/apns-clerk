    ...
    def load_context(self, cert_string=None, cert_file=None, key_string=None, key_file=None, passphrase=None):
        ...
        if cert_file:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("Certificate provided as file: %s", cert_file)

            with open(cert_file, 'rb') as fp:
                cert_string = fp.read()
        else:
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("Certificate provided as string")
        ...
        if not key_string and not key_file:
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("Private key provided with certificate %s %s passphrase",
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("Private key provided as file without passphrase: %s", key_file)

            context.use_privatekey_file(key_file, OpenSSL.crypto.FILETYPE_PEM)
        else:
            if key_file:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug("Private key provided as file with passphrase: %s", key_file)
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug("Private key provided as string %s passphrase", 'with' if passphrase is not None else 'without')
...
class Connection(BaseConnection):
    ...
    def _open_connection(self, timeout):
        ...
        try:
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("Opening POSIX socket/pyOpenSSL connection to %r", self.address)
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("SSL handshaking to %r completed", self.address)
        except:
            logger.warning("Failed to establish socket/SSL connection to %r", self.address, exc_info=True)
            self.close()
            raise
    ...
    def _connect_and_handshake(self):
        ...
        # to block for that timeout. So we use an infinitely blocking socket here,
    ...
    def write(self, data, timeout):
        ...
        # any value except None. So it is either block forever or immediate
        ...
        while True:
            try:
                ...
                if timeout is not None:
                    ...
                    if timeout - waited > 0:
                        before = time.time()
                        _, can_write, _ = select.select((), (self._socket, ), (), timeout - waited)
                        waited += time.time() - before

                        if can_write and timeout - waited >= 0:
            ...
            logger.warning("Write timeout %r it too short for buffer %s", timeout, len(data))
    ...
    def peek(self, size):
        ...
        if pending > 0:
            ...
            can_read, _, _ = select.select((self._socket, ), (), (), 0)  # poll
        ...
        while True:
            try:
                ...
                if timeout is not None:
                    ...
                    if timeout - waited > 0:
                        before = time.time()
                        can_read, _, _ = select.select((self._socket, ), (), (), timeout - waited)
                        waited += time.time() - before

                        if can_read and timeout - waited >= 0:
                            # buffer ready for writing, try again
                            self._socket.settimeout(timeout - waited)
                            continue
